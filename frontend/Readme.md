...
Используется для отображения списка доступных вариантов аренды на главной странице приложения.

#### `GET /offers/:offerId`

Получение полной информации о конкретном предложении.

Используется на странице карточки предложения.
...
Требует авторизации пользователя и прав владельца предложения.
...
Получение списка премиальных предложений для выбранного города.

Используется для отображения премиальных предложений.
...
Используется на странице предложения.
...
### Избранные предложения (favorite)

#### `GET /offers/favorite`

Получение списка избранных предложений текущего пользователя.
...
В ответе сервер возвращает JWT-токен, который сохраняется на клиенте.
